...
int main(int argc, char *argv[])
{
	...
	if (argc != 3){
		printf("Usage : %s <target hostname/IP> <port>n", argv[0]);
		exit(-1);
	}
	...
	printf("Target IP: %s port: %u.n", argv[1], atoi(argv[2]));
	while(1) {
		...
		unsigned long sum = (ipheader->saddr >> 16) + (ipheader->saddr & 0xffff) +
		(ipheader->daddr >> 16) + (ipheader->daddr & 0xffff) + 1536 +
